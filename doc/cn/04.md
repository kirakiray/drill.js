...
如上案例 `require('a')` 会引用 `js/a.js` 文件，而 `require('b -r')` 则会引用根目录的 `b.js` 文件，无视 `baseUrl:"js/"` 的设定
...
如上案例 `require('a')` 会引用 `js/a.js` 文件，而 `require('b -pack')` 会载入 `js/b/b.js`；

`-pack` 参数作为 `drill` 的**包**，包的自动文件是包目录内和包同名的js文件；

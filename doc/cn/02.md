...
默认的情况下，`require` 都是以根目录为准；但在模块内，我们可以使用相对目录；
...
```javascript
define(async (require) => {
    // let d1 = await require('js/definedir/d1');
...
<body>
    <script>
    (async ()=>{
        // let [d1,d2] = await require('js/definedir/d1','js/definedir/d2');

        let d1 = await require('js/definedir/d1');

        console.log(d1.val);

        let d2 = await require('js/definedir/d2');
